...
def test_auth_signup(session, test_fixture_user):
    res = session.query(User).filter(User.email == test_fixture_user["email"]).first()
    assert res.email == test_fixture_user["email"]
    session.close()
...
def test_auth_verify_user_token(test_fixture_user, user_token_details, client):
    ...
    assert user_id == test_fixture_user['id']
    assert user_email == test_fixture_user['email']
